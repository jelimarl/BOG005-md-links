...
function cli(route, args) {
  if (args.length === 3) {
    mdLinks(route, { validate: false }).then((value) => {
      if (value === 'There are no .md files' || value === 'There are no links' || value === 'That path does not exist') { console.log(value) }
  ...
  else { console.log('Write valid options after your path') }
}
